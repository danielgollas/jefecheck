...
**Card pattern (mirrors FX panel §23).** `PlaylistPanel_Qt` hosts a `QListWidget` (`playlist.list`) whose items are `PlaylistItemCard` custom `QFrame` widgets set via `setItemWidget`. Each card shows a drag-handle glyph (affordance only — see reorder below), a name label (`playlist.card.<i>.name`), a chevron (`playlist.card.<i>.chevron`) that collapses/expands the per-track detail section, and a per-card remove button (`playlist.card.<i>.remove`). The card frame itself is `playlist.card.<i>`. Expanded detail rows are rebuilt from `jefe::qt::getPlaylistItemDetail(index)` — one row per loaded track showing filename (Compact vs Full-paths toggle), range, and scale. Cards connect `loadRequested` → `loadRow`, `removeRequested` → `removePlaylistItem` + `refreshList`, `toggleExpandRequested` → collapse the previously expanded card then expand the new one.
