...
**Carried-over JEF-24 hardening TODOs (from the JEF-23 review — still open, and they BLOCK internet exposure).** The wire layer is bounds-safe for *shape*, but these remain: **path traversal** in `unserializeFX`/`unserializeLUT` (a peer-supplied filename is used to write/read a file — an attacker could escape the intended dir); **wire-index validation** (received quad/track/FX indices are trusted, not range-checked against the local managers); **envelope/frame-id check** (no per-peer replay/spoof guard on message type vs sender role); **`ftell` guard** in the LUT/DPX read paths (a negative/failed `ftell` isn't checked before use). None matter on a trusted LAN (this ticket's scope); all must be closed before the JEF-25 cloud coordinator ever carries a session across the public internet.
...
**Still-open items.** (1) **Real cloud deploy is the operator's step (JEF-25/26)** — verified locally only; no live wss:// endpoint is provisioned here. (2) **Password is NOT enforced** — the phase-1 coordinator gates by code/token, not a password field; `configureCoordinator` carries `password` but the protocol has no slot for it, so **the session code IS the gate** (residual: anyone with the code joins; document, don't fake enforcement). (3) The **co-located-coordinator libdatachannel bug** (above) is worked around by process isolation, not fixed — it mirrors real deployment (remote coordinator) so it isn't a blocker. (4) The JEF-24 internet-exposure hardening TODOs (§33: path traversal, wire-index validation, replay/spoof guard, unauthenticated signaling) STILL BLOCK public-internet exposure and must be closed before any session crosses it.
