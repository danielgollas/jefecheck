...
- **CI bundling:** release packaging downloads a GPL static ffmpeg (`eugeneware/ffmpeg-static` `b6.1.1`) per platform into the bundled location; `packaging/ffmpeg-NOTICE.txt` documents the license. `JEFECHECK_FFMPEG_BIN` overrides the download for offline packaging. `create_macos_bundle.sh` now accepts the binary from either `build/jefecheck` or the `.app` bundle.
