...
- **The timeline range is freely editable, not pinned to track length.** The in/out spinboxes are un-clamped (max ~10M, not capped to `to`), and `gfcPlaybackManager::setOutPoint` grows `to` when the out point exceeds it — so pushing the out point past the loaded tracks extends the visible/playable timeline (e.g. to view offset frames). Loading still fits `to` to the longest track initially.
