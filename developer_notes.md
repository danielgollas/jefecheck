...
## 36. Session-health stats (JEF-30)

**Files:** `src/gfcTransport.h` (`PeerStats`, `ITransport::peerStats()`), `src/gfcWebRtcTransport.{h,cpp}`, `src/gfcRakNetTransport.{h,cpp}`, `src/gfcnetworkclient.h`/`gfcnetworkserver.h` (per-role forwarding), `src/gfcnetworkmanager.{h,cpp}` (`peerStats()`/`peerNickname()`), `src/qt/SequenceLoadBridge_qt.{h,cpp}` (`RemotePeerStat`, `jefe::qt::remotePeerStats()`), `src/qt/RemotePanel_qt.{h,cpp}` (dialog indicator), `src/main_qt.cpp` (`--stats-test`). Read-only observability layered on top of §33's WebRTC transport and §26's remote dialog — no wire-format or transport-behavior change.

**`PeerStats` seam.** `struct jefe::net::PeerStats { PeerId peer; long rttMs=-1; uint64_t bytesSent=0, bytesReceived=0; enum class Path{Unknown,Direct,Relay} path; bool connected=false; }` and a defaulted `virtual std::vector<PeerStats> ITransport::peerStats() { return {}; }` — only WebRTC/RakNet override it.

**libdatachannel read (`WebRtcTransport::peerStats()`).** Same two-phase lock discipline as `send()`/`drainAssetQueue` (§33/§35): under `Impl::mtx`, copy each peer's `{PeerId, pc shared_ptr, open}` into a local snapshot, release the lock, THEN query libdatachannel — `pc->rtt()` (`nullopt`→`-1`), `pc->bytesSent()/bytesReceived()`, `pc->getSelectedCandidatePair(&local,&remote)` → classify by the **remote** candidate's `type()`: `Relayed`→`Path::Relay`, `Host`/`ServerReflexive`/`PeerReflexive`→`Path::Direct`, else `Unknown`. `connected` = the STATE ("jefe") channel's `open` flag (same liveness source as `connectionCount()`). Every libdatachannel call is `try/catch(...)`-wrapped and degrades to the field's default — best-effort, never throws out of `peerStats()`.

**RakNet degradation.** No WebRTC-style stats exist, so `RakNetTransport::peerStats()` enumerates connected systems via `GetConnectionList` (null-buffer count query, then a sized fetch clamped to the returned capacity — a peer connecting between the two calls can't overrun the buffer) and emits one basic entry per peer: `connected=true, path=Unknown, rttMs=-1, bytes=0`.

**Forwarding.** `gfcNetworkManager::peerStats()` forwards to the **active** transport — `isServer ? server.peerStats() : client.peerStats()` (empty when not `connected`): a host's stats are the server transport's view of every joined peer (including its own loopback client, §26/§33); a joiner's are the client transport's single peer (the host). `gfcNetworkServer`/`gfcNetworkClient` each expose a thin inline `peerStats()` forwarding to `transport_`; the server also adds `nicknameForPeer(PeerId)` (looks up `nickNameAddressMap` — server role only).

**Bridge getter (`jefe::qt::remotePeerStats()`).** TU-safe per §1 (lives in `SequenceLoadBridge_qt.cpp`, not the Qt/GL dialog TU) and honors `gCloudConnectInFlight` exactly like the sibling getters (`remoteParticipants`/`remoteStatusText`/etc., §34) — returns empty during a cloud connect. Maps each `PeerStats` to a UI-friendly `RemotePeerStat{name, rttMs, kbps, bytes, path, connected}`: `name` resolves the nickname via `peerNickname()` when known, else `"peer <id>"`; `bytes` is the running `bytesSent+bytesReceived` total (kbps math lives in the dialog, not the bridge — `kbps` is left `0.0` here); `path` maps `Direct`→`"direct"`, `Relay`→`"relay"`, else `"n/a"`.

**Dialog indicator (`RemotePanel_qt`).** Driven by the existing `refreshConnectionState()` tick (§26). Row *widgets* rebuild only when the participant count changes (matching the dialog's existing incremental-refresh discipline); the health fields inside each row (dot/rtt/kbps/path) are updated in place every tick via cheap `QLabel::setText`/`setStyleSheet` — no relayout churn. kbps is derived from a per-peer (keyed by display name) byte-delta sample: `dtSec = duration<double>(now-lastTs)`, only recomputed once `dtSec >= 0.15` (avoids noisy ~60Hz-tick division and a too-short interval), a negative delta (counter reset on reconnect) clamps to 0 rather than going negative. Samples (`peerHealthSamples_`) are cleared wholesale on every disconnect transition so a later reconnect starts clean. Colors: dot/path green (`direct`) / amber (`relay (TURN)`) / gray (`n/a`/no-stats); RTT green `<100ms`, amber `>=100ms`, gray if unknown (`rttMs<0`). A participant with no matching `RemotePeerStat` (RakNet, pre-connect, or the joiner-side host-nickname gap below) renders a plain gray row — `"connected"` or blank — never a fabricated number. Object names: `remote.health.{participantRow,dot,name,rtt,kbps,path}` (dotted-leaf scheme, per `tests/ui/jefecheck/locators.py`; reused per row like the existing `chat_*` widgets, not unique per participant — this dialog has no per-row automated locator today).

**`--stats-test` harness.** Reuses the §33 two-process WebRTC harness (`--remote-test-webrtc`'s re-exec + split-phase loopback pattern): host + its own loopback client come up first (`remoteTestServerStart`), then a second peer is spawned (`--remote-test-webrtc-peer` child role) for real cross-process traffic and a second candidate pair, then `remoteTestServerSettleForPlay` lets the peer's play toggle flow. A bounded retry loop (≤4s, `pumpNetwork()` each iteration) polls `remotePeerStats()` until a peer shows `connected && bytes>0 && path∈{direct,relay}` — rtt is deliberately NOT required (localhost rtt is frequently `0`/`nullopt`→`-1`). Prints `STATS-TEST: peers=N rtt=<ms> bytes=<n> path=<direct|relay|unknown>`, exits 0 iff a real-stats peer was observed. Observed deterministic output: `peers=2 rtt=1-2 bytes=677 path=direct`.

**Cloud half (separate repo, not detailed here).** CloudWatch dashboard + alarms for the coordinator (signaling Lambda `Invocations`/`Errors`/`Duration`/`Throttles`, API Gateway WS connect/message counts, DynamoDB consumed-capacity/throttling, coturn EC2 `NetworkIn`/`NetworkOut` as the TURN bandwidth/credit-metering signal, plus an `ActiveSessions` custom-metric namespace documented as a contract the coordinator Lambda would emit) live in `~/projects/jefecheck-coordinator` on `feature/coordinator`, `tofu validate`-clean, deploy deferred (same honest boundary as JEF-25/26, §34) — see that repo, not this file, for detail.

**Known limitations.** (1) **Joiner-side host row has no stats.** `peerNickname()`/name resolution is server-role only (`nicknameForPeer` reads `nickNameAddressMap`, which only the server populates); on the client side `RemotePeerStat::name` falls back to `"peer <id>"`, which never matches `remoteParticipants()`'s resolved name, so the dialog's name-keyed lookup misses and the host's row renders as a plain gray "connected" line with no RTT/kbps/path — a display gap, not a bug, flagged as follow-up work (client-role nickname resolution wasn't scoped to T1/T2). (2) **Per-peer kbps sample isn't pruned on a single-peer leave** in a multi-peer session — `peerHealthSamples_` is keyed by name and only cleared wholesale on full disconnect; a departed peer's stale entry just sits unused (bounded by session peer-count cardinality, not a leak). (3) **localhost rtt is often `0`/`nullopt`→`-1`** — `--stats-test` deliberately does not gate on rtt for this reason; a same-host loopback test is not representative of real network RTT.
