...
{	
		...
		float d= 2.0*top.g - top.r- top.b; //simplified distance function
			
		float clipMatte;
		clipMatte=smoothstep(astart,astart+range,d);
				
		top = vec4(top.r,min(top.g,top.r),top.b,top.a)*spillSupress + top*((spillSupress-1.0)*(spillSupress-1.0));
